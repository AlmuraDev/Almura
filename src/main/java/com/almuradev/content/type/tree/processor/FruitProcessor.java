...
public final class FruitProcessor implements AbstractTreeProcessor {
    ...
    private static final ConfigTag TAG = ConfigTag.create(TreeConfig.FRUIT);
    ...
    public void processTagged(final ConfigurationNode config, final Tree.Builder builder) {
        ...
        config.getNode(TreeConfig.Block.CHANCE).getChildrenList().forEach(child -> heights.add(new DoubleRangeFunctionPredicatePair<>(
                BIOMES.allowingSingleAlwaysTrueDefault(foundDefault, child.getNode(TreeConfig.Block.Chance.BIOME), "fruit"),
                DoubleRanges.deserialize(child).orElseThrow(() -> new IllegalArgumentException(TreeConfig.Block.CHANCE +  "is missing"))
        )));
        builder.fruit(LazyBlockState.parse(config.getNode(TreeConfig.Block.BLOCK)), heights);
    }
}
