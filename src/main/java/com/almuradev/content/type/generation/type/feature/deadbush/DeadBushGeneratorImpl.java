...
public final class DeadBushGeneratorImpl implements DeadBushGenerator, Witness {
    private final List<String> worlds;
    private final List<DoubleRangeFunctionPredicatePair<Biome>> biomes;
    private final List<LazyBlockState> requires;
    private final Delegate<DeadBush> deadBush;

    DeadBushGeneratorImpl(final List<String> worlds, final List<DoubleRangeFunctionPredicatePair<Biome>> biomes, final List<LazyBlockState> requires,
            final Delegate<DeadBush> deadBush) {
        this.worlds = worlds;
        this.biomes = biomes;
    ...
    public void decorate(final DecorateBiomeEvent.Decorate event) {
        if (event.getType() != DecorateBiomeEvent.Decorate.EventType.DEAD_BUSH || event.getResult() == Event.Result.DENY) {
            return;
        }
        ...
        if (random.nextDouble() <= (chance.random(random) / 100d)) {
            ...
            ((DeadBushFeature) this.deadBush.require()).generate(world, random, pos, this.requires);
