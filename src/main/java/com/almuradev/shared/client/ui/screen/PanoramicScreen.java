 ...
 * All Rights Reserved.
 */
package com.almuradev.shared.client.ui.screen;
...
public abstract class PanoramicScreen extends SimpleScreen {
    ...
    static {
        final Path basePath = Constants.FileSystem.PATH_ASSETS_ALMURA_30.resolve("textures/gui/title/background");
                ...
                for (Path path : daypartPaths) {
                    if (matcher.matches(path)) {
                        ...
                        try {
                            ...
                            Almura.instance.logger.warn("Unable to load panoramic for [" + path + "]", e);
                        }
                    }
                }
            } catch (IOException e) {
                Almura.instance.logger.warn("Unable to load panoramics.", e);
            }
        }
    }
    ...
    public PanoramicScreen(@Nullable SimpleScreen parent) {
        ...
        if (bundles.isEmpty() || daypartBundles == null || daypartBundles.isEmpty()) {
            Almura.instance.logger.warn("No custom panoramics found. Defaulting to vanilla panoramic.");
