...
public final class ClientboundAvailableExchangesResponsePacket implements Message {

    @Nullable public Set<Exchange> exchanges;

    public ClientboundAvailableExchangesResponsePacket() {
    }

    public ClientboundAvailableExchangesResponsePacket(@Nullable final Set<Exchange> exchanges) {
    ...
    public void readFrom(final ChannelBuf buf) {
        ...
        if (count > 0) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                try {
                    this.exchanges.add(PacketUtil.fromBytes(buf.readBytes(length)));
